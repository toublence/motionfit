...
- 운동 파이프라인은 원본 영상, 이미지, 카메라 프레임을 앱 파일이나 SQLite에 저장하지 않는다.
- 원본 영상이나 프레임을 Flutter MethodChannel/EventChannel로 전달하지 않는다.
- 앱 코드에는 영상·프레임·이미지를 서버로 업로드하는 경로가 없다.
- 횟수 감지에 사용한 landmark와 world landmark는 런타임 처리 후 운동 DB에 저장하지 않는다.
- Body Progress는 예외다. 사용자가 촬영 버튼을 누른 시점에만 JPEG 1장을 기기 내부 비공개 디렉터리에 저장한다. 갤러리나 사진 라이브러리에 쓰지 않고 업로드하지 않는다.
- Form Progress는 예외다. 한 세션당 가장 점수가 높은 반복 1회의 normalized landmark 좌표(x, y, confidence)만 저장한다. 픽셀, 영상, world landmark는 저장하지 않는다.
...
| Body Progress 사진(JPEG) | 네이티브 still capture → 앱 비공개 디렉터리 | 예, 사용자가 촬영한 경우만 | 아니요 |
| Body Progress 사진 색인(날짜·구도·파일명) | SQLite `motionfit.db` | 예 | 아니요 |
| Form Progress 대표 자세 landmark(x, y, confidence) | SQLite 운동별 DB | 예, 세션당 1행 | 아니요 |
...
운동 세션·세트·반복 테이블에는 영상·이미지·픽셀·landmark를 위한 DB column이 없다. 관찰할 수 없는 자세 지표는 nullable 값 또는 `notObservable` 상태로 처리하며 추측값을 저장하지 않는 것이 원칙이다.

Body Progress와 Form Progress는 별도 테이블(`body_progress_photos`, `form_pose_snapshots`)을 사용한다. 두 테이블 모두 BLOB이나 픽셀 데이터를 담지 않는다. 사진은 파일명으로만 참조하고 실제 JPEG는 앱 비공개 디렉터리(iOS Application Support, Android `noBackupFilesDir`)에 두며 두 경로 모두 OS 백업에서 제외한다. 이 구분은 `sqlite_workout_repository_test.dart`의 스키마 가드 테스트가 검증한다.
...
- Form Progress 대표 자세는 세션 행에 foreign key로 묶여 있어 세션이 삭제되면 함께 삭제된다.
- Body Progress 사진은 화면에서 개별 삭제할 수 있고, 삭제 시 DB 행과 JPEG 파일을 함께 제거한다. 설정의 일괄 데이터 삭제에는 아직 포함되지 않았다. 출시 전에 연결해야 한다.
...
| 권한/기능 | 요청 시점 | 목적 |
|---|---|---|
| 카메라 | 운동 시작 시, Body Progress 촬영 시 | 온디바이스 자세 감지와 프리뷰, 사용자가 요청한 Body Progress 사진 촬영 |
...
마이크, 사진 라이브러리, 미디어, 저장소, 위치, 연락처, 건강 데이터 권한은 현재 요청하지 않는다. Body Progress 사진도 사진 라이브러리를 거치지 않고 앱 비공개 디렉터리에만 저장하므로 추가 권한이 필요 없다. 권한이 거부되어도 기록과 설정은 계속 사용할 수 있다.
