...
import 'package:motionfit_squat/features/plank/workout/data/native_pose_engine.dart';
...
import 'package:motionfit_squat/features/plank/workout/domain/models/workout_journal.dart';
...
import 'package:motionfit_squat/features/plank/workout/domain/services/pose_feedback_classifier.dart';
import 'package:motionfit_squat/features/plank/workout/domain/services/pose_landmark_smoother.dart';
...
class WorkoutSessionController extends Notifier<WorkoutSessionState> {
  ...
  void _handleRepCompleted(RepMotionTrace trace) {
    ...
    final preferences = ref.read(preferencesControllerProvider);
    final remainingSeconds = state.targetReps - nextSetReps;
    final announceCheckpoint =
        nextSetReps == 1 || nextSetReps % 5 == 0 || remainingSeconds <= 3;
    if (preferences.hapticsEnabled && announceCheckpoint) {
  ...
  Future<bool> _persistCompletedRep(
    ...
    try {
      ...
      if (issueToCoach != null) {
        ...
        } else if (trace.repSequence % 15 == 0) {
          _enqueueCoach(
            CoachMessageType.encouragement,
