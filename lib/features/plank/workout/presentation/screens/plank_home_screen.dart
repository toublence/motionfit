...
import 'package:motionfit_squat/features/plank/records/domain/retention_metrics.dart';
...
import 'package:motionfit_squat/features/plank/workout/presentation/workout_preparation_launcher.dart';

class PlankHomeScreen extends ConsumerStatefulWidget {
  const PlankHomeScreen({super.key});

  @override
  ConsumerState<PlankHomeScreen> createState() => _PlankHomeScreenState();
}

class _PlankHomeScreenState extends ConsumerState<PlankHomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final plan = ref.read(preferencesControllerProvider).plankLastWorkoutPlan;
  ...
  Widget build(BuildContext context) {
    ...
    final plan = preferences.plankLastWorkoutPlan;
          ...
          children: [
            ...
            if (latest != null)
              TextButton.icon(
                ...
                onPressed: () =>
                    context.push('/records/plank/session/${latest.session.id}'),
