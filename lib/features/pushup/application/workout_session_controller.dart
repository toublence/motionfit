...
import 'package:motionfit_squat/features/pushup/data/native_pose_engine.dart';
...
import 'package:motionfit_squat/features/pushup/domain/services/pose_feedback_classifier.dart';
import 'package:motionfit_squat/features/pushup/domain/services/pose_landmark_smoother.dart';
...
class WorkoutSessionController extends Notifier<WorkoutSessionState> {
  ...
  PushupRepDetector? _repDetector;
  final FormAnalyzer _formAnalyzer = const PushupFormAnalyzer();
  ...
  Future<void> start(
    ...
    try {
      ...
      state = state.copyWith(
        ...
        phase: calibrated ? PushupPhase.ready : PushupPhase.calibrating,
  ...
  Future<void> _startEngines(WorkoutCoachMessages messages) async {
    ...
    _repDetector = PushupRepDetector();
  ...
  void _onPoseFrame(PoseFrame frame) {
    ...
    try {
      ...
      state = state.copyWith(
        trackingState: frame.trackingState,
        phase: PushupPhase.trackingLost,
        overlayLandmarks: overlayLandmarks,
        previewTransform: frame.previewTransform,
        previewMirrored: frame.mirrored,
        previewRotationDegrees: frame.rotationDegrees,
  ...
  void _saveFormPoseSnapshot({
    ...
    unawaited(
      ref
          ...
          .captureIfNeeded(exercise: ExerciseType.pushup, snapshot: snapshot),
    );
  }
  ...
  void _recordDebugFrame(
    ...
    final values = <String, Object?>{
      'kneeAngle': null,
      'hipAngle': null,
      'elbowAngle': metrics?.kneeAngle,
      'torsoAngle': metrics?.torsoLeanDegrees,
      'bodyLineAngle': metrics?.hipAngle,
      'hipDrop': null,
      'shoulderDrop': metrics?.hipDrop,
      'velocity': metrics?.hipVelocity,
      'hipRelativeMovement': metrics?.shoulderHipRelativeMovement,
      'elbowFlare': metrics?.leftHeelLift,
      'elbowImbalance': metrics?.kneeAlignmentDeviation,
      ...
      'descentElbowFlexDegrees': config.descentKneeFlexDegrees,
      'descentBodyLineFlexDegrees': config.descentHipFlexDegrees,
      'descentShoulderDrop': config.descentHipDrop,
