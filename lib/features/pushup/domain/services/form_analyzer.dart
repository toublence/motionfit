...
class PushupFormConfig {
  const PushupFormConfig({
    this.minimumConfidence = 0.70,
    this.minimumPersistence = 0.30,
    this.goodDepthKneeAngle = 100,
    this.minimumGoodHipDrop = 0.055,
    this.maximumTorsoLeanDegrees = 18,
    this.maximumHeelLift = 0.06,
    this.maximumKneeAlignmentDeviation = 0.12,
    this.maximumBalanceDeviation = 0.10,
    ...
    this.lockoutKneeAngle = 155,
    this.lockoutHipAngle = 150,
  });
...
class PushupFormAnalyzer implements FormAnalyzer {
  const PushupFormAnalyzer({this.config = const PushupFormConfig()});

  final PushupFormConfig config;
  ...
  FormAnalysisResult analyze(RepMotionTrace trace) {
    ...
    final finish = samples.last;
    final lockoutObservable =
        finish.leftKneeAngle != null || finish.rightKneeAngle != null;
    final lockoutPassed = finish.kneeAngle >= config.lockoutKneeAngle;
    metrics[FormMetricType.lockout] = !lockoutObservable
        ? _notObservable(FormMetricType.lockout)
        : FormMetricResult(
            type: FormMetricType.lockout,
            status: finish.confidence < config.minimumConfidence
                ? FormMetricStatus.insufficientConfidence
                : lockoutPassed
                ? FormMetricStatus.passed
                : FormMetricStatus.needsAttention,
            score: finish.confidence < config.minimumConfidence
                ? null
                : _scoreHigherIsBetter(
                    finish.kneeAngle,
                    worst: 115,
                    good: config.lockoutKneeAngle,
                  ),
  ...
  FormMetricResult _assessThreshold({
    ...
    required List<PushupMetrics> samples,
  ...
  FormMetricResult _assessOptionalThreshold({
    ...
    required bool observable,
    required List<PushupMetrics> samples,
    required double? Function(PushupMetrics) value,
  ...
  double _meanVelocityJerk(List<PushupMetrics> samples) {
  ...
  bool _isContinuous(List<PushupMetrics> samples) {
  ...
  double _meanConfidence(List<PushupMetrics> samples) =>
      samples.fold<double>(0, (sum, sample) => sum + sample.confidence) /
      samples.length;
  ...
  CameraAngle _dominantAngle(List<PushupMetrics> samples) {
