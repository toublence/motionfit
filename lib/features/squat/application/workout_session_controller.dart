...
class WorkoutSessionController extends Notifier<WorkoutSessionState> {
  ...
  // Squats are normally filmed with the whole body relatively small in frame.
  // Start with the more accurate model, then fall back only when the measured
  // device latency cannot sustain a useful real-time cadence.
  PoseModelQuality _activeModelQuality = PoseModelQuality.full;
  ...
  void _saveFormPoseSnapshot({
    ...
    unawaited(
      ref
          ...
          .captureIfNeeded(exercise: ExerciseType.squat, snapshot: snapshot),
    );
  }
  ...
  void _resetTiming({bool preservePreparedRuntime = false}) {
    ...
    if (!preservePreparedRuntime) {
      ...
      _activeModelQuality = PoseModelQuality.full;
  ...
  void _observeInferenceLatency(int milliseconds) {
    ...
    final desiredFps = average < 25
        ...
        ? 20
        : 15;
    final desiredModel = average >= 65 && !_videoRecordingActive
        ? PoseModelQuality.lite
        : _activeModelQuality;
    if (desiredFps == _targetInferenceFps &&
        desiredModel == _activeModelQuality) {
      return;
    }
    unawaited(
      _applyPerformancePolicy(
        targetFps: desiredFps,
        modelQuality: desiredModel,
      ),
    );
  }

  Future<void> _applyPerformancePolicy({
    required int targetFps,
    required PoseModelQuality modelQuality,
  }) async {
    final engine = _poseEngine;
    ...
    try {
      ...
      if (modelQuality != _activeModelQuality) {
        await engine.setModelQuality(modelQuality);
        _activeModelQuality = modelQuality;
      }
      _inferenceLatencies.clear();
    } on Object {
