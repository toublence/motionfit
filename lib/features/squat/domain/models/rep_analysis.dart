...
class RepAnalysis {
  ...
  static List<RepAnalysis> fromRecords({
    ...
    for (var index = 0; index < records.length; index++) {
      ...
      final bottom = record.bottomAt == null &&
              record.videoBottomMilliseconds == null
          ? null
          : _elapsed(
      ...
      final primaryIssue = record.primaryIssue ??
          (record.detectedIssues.isEmpty ? null : record.detectedIssues.first);
      final assessed = record.overallFormScore != null ||
      ...
      final resolvedPreviousIssue = previous != null &&
          previousRecord != null &&
  ...
  static bool _resolved(FormIssue? previousIssue, RepRecord current) =>
      switch (previousIssue) {
        FormIssue.insufficientDepth =>
          current.depthQuality == RepQuality.good,
        FormIssue.excessiveTorsoLean =>
          current.upperBodyQuality == RepQuality.good,
        FormIssue.kneeAlignment =>
          current.kneeAlignmentQuality == RepQuality.good,
        _ => false,
      };
}
