...
class SystemTtsCoachEngine implements CoachVoiceEngine {
  ...
  Future<bool> configure({required String locale, required double rate}) async {
    ...
    final canonicalLanguage =
        baseLanguage == 'zh' && normalizedLocale.toLowerCase().contains('hant')
        ? 'zh-TW'
        : _languageTags[baseLanguage];
    // Only app languages are mapped; anything else has no voice.
